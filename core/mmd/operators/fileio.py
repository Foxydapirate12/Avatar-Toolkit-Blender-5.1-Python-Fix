...
# from .. import auto_scene_setup  # Not used in Avatar Toolkit
from ..core.camera import MMDCamera
from ..core.light import MMDLight
from ..core.model import FnModel, Model
# from ..core.pmd import importer as pmd_importer  # PMD not supported in Avatar Toolkit
# from ..core.pmx import exporter as pmx_exporter  # PMX export not supported in Avatar Toolkit
from ..core.pmx import importer as pmx_importer
# from ..core.vmd import exporter as vmd_exporter  # VMD export not supported in Avatar Toolkit
from ..core.vmd import importer as vmd_importer
# from ..core.vpd import exporter as vpd_exporter  # VPD not supported in Avatar Toolkit
# from ..core.vpd import importer as vpd_importer  # VPD not supported in Avatar Toolkit
...
def log_handler(log_level, filepath=None):
    if filepath is None:
        ...
        handler = logger.FileHandler(filepath, mode="w", encoding="utf-8")
...
class ImportPmx(Operator, ImportHelper, PreferencesMixin):
    ...
    def _do_execute(self, context):
        # Avatar Toolkit uses its own logger system, skip log configuration
        # logger level and handlers are managed globally
        
        try:
            importer_cls = pmx_importer.PMXImporter
            if re.search(r"\.pmd$", self.filepath, flags=re.IGNORECASE):
                # PMD format not supported in Avatar Toolkit
                self.report({"ERROR"}, "PMD format is not supported. Please convert to PMX format.")
                return {"CANCELLED"}
        ...
        try:
            ...
            for file in self.files:
                ...
                for i in selected_objects:
                    importer.assign(i)
                logger.info(" Finished importing motion in %f seconds.", time.time() - start_time)

            if self.update_scene_settings:
                # auto_scene_setup.setupFrameRanges()  # Not available in Avatar Toolkit
                # auto_scene_setup.setupFps()  # Not available in Avatar Toolkit
                pass
...
# VPD format not supported in Avatar Toolkit - classes below are disabled
class ImportVpd(Operator, ImportHelper, PreferencesMixin):
    _is_registered = True  # Prevent auto_load from registering this
...
class ExportPmx(Operator, ExportHelper, PreferencesMixin):
    _is_registered = True  # Prevent auto_load from registering this (PMX export not supported)
...
class ExportVmd(Operator, ExportHelper, PreferencesMixin):
    _is_registered = True  # Prevent auto_load from registering this (VMD export not supported)
    ...
    def execute(self, context):
        ...
        try:
            ...
            logger.info(" Finished exporting motion in %f seconds.", time.time() - start_time)
        except Exception:
            logger.exception("Error occurred")
            err_msg = traceback.format_exc()
            self.report({"ERROR"}, err_msg)
...
class ExportVpd(Operator, ExportHelper, PreferencesMixin):
    _is_registered = True  # Prevent auto_load from registering this (VPD not supported)
